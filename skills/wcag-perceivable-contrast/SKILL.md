...
looks too pale to you, say nothing here. It is already covered, and a duplicate
reaches the ledger as a second finding on the same element.
...
another. Nothing here is about ratios: a distinction drawn in two vividly
different hues at 7:1 still fails when hue is all there is.

The second cue must exist at rest, without hover or focus. Shape, an icon,
underline, weight, size, position, border style, a pattern fill, or plain text.

### How to test it

1. List every place colour carries meaning: links inside running text,
   required-field markers, validation states, status badges, chart series and
   legends, calendar availability, diff highlighting, map keys, the current tab
   or step of a progress indicator.
...
4. Links in body text are the special case. Colour alone is permitted only when
   the link colour reaches 3:1 against the surrounding body text **and** a
   further cue appears on hover and focus. That exception needs a measured
   ratio; with no measurement, describe what you see and use FLAG.
5. Charts: if legend swatches differ only in hue and the plot has no direct
   labels, no differing markers and no dash or fill patterns, the series cannot
   be told apart.
...
- A required field indicated only by a red label - no asterisk, no "(required)",
  no instruction saying that red means required.
...
- A five-series line chart separated only by stroke colour, with a colour-swatch
  legend and no direct labels or dash patterns.
- A status column where paid rows are green and overdue rows are red, with the
  same word or no word in the cell.
- A step indicator whose current step differs only in fill colour - no ring, no
  numeral change, no "Step 2 of 4" text.
...
- A red border **plus** "Enter a valid postcode". Redundant colour is correct use
  of colour.
...
- A chart whose lines are labelled at their right-hand ends, or whose markers
  differ in shape, even when the colours differ too.
...
Not the whole component, not the whole graphic. Exempt: inactive or disabled
components, appearance determined entirely by the user agent and not modified by
the author, and logotypes.

### How to test it

1. **Trust measured ratios supplied by the application over your own impression
   of the pixels.** Where you have a measurement it rules, whether or not the
   rendering looks fine. Where you have none, describe what you see and use
   FLAG. Never state a ratio you did not measure.
2. For each control ask which pixels say "this is a control" and which say what
   state it is in. If the answer is a 1px border, that border must reach 3:1
   against the page behind it. If it is a filled block, the fill against the page.
3. Check both adjacencies of a focus indicator - the ring against the page, and
   the ring against the component it surrounds. Size and obscuring are 2.4.11;
   contrast is here.
...
- `input` with `border: 1px solid #e0e0e0` on a white field, around 1.3:1, so the
  boundary is invisible to a low-vision user.
- A focus ring of `outline: 2px solid #b3d4fc` on white, under 2:1.
- A toggle whose off-state track is `#eeeeee` on white with no border, so neither
  the control nor its state can be seen.
- A checkbox drawn as a `#dddddd` hairline square.
- An icon-only button whose glyph is `#cfcfcf` on a white toolbar.
- Pie slices in adjacent pale tints, no separating stroke and no labels, where
  separating the slices is how the chart is read.

### False positive - do not report

- Disabled or inactive controls. Explicitly exempt.
- Decorative pixels: gridlines, background patterns, watermarks, drop shadows,
  hairline row dividers. Nothing required to understand the content runs
  through them.
- A logotype or brand wordmark.
- An unstyled browser-default `select` or checkbox. User-agent appearance is
  exempt.
...
Nothing flashes more than three times in any one second period, unless the
flashing is below both thresholds. A flash is a **pair of opposing changes** in
relative luminance.

- General flash threshold: opposing changes of 10% or more of maximum relative
  luminance, with the darker state below 0.80, over an area larger than 25% of
  10 degrees of the visual field - roughly 341 x 256 px on a 1024 x 768 display
  at typical viewing distance.
- Red flash threshold: any pair of opposing transitions involving a saturated
  red, whatever the luminance change.

This covers the whole page, third-party embeds and advertising included.

### How to test it

1. Inventory what can flash: `video`, animated GIF, APNG and animated WebP,
   `canvas`, CSS `animation` or `transition` loops on `opacity`,
   `background-color`, `filter` or `visibility`, autoplaying embeds and ads,
   strobing loading states, game content.
2. For CSS, do the arithmetic rather than watching. Count opposing luminance
   ...
   and the keyframe stops. A two-stop keyframe at `animation: blink 0.15s
   infinite` is roughly 6.7 flashes per second.
...
6. If you cannot obtain frames - DRM, a cross-origin iframe, autoplay blocked -
   ...
   nothing, and say which.

### Genuine failure

- A hero video with a strobe or camera-flash sequence exceeding three flashes a
  second across most of the viewport, with no warning and no way to stop it.
...
- A cross-fade, a scroll-triggered fade-in, a hover transition. None are opposing
  luminance flashes.
- Fast-cut video editing with no large luminance swing.
- Parallax, carousels, marquees and other motion that does not flash. That is
  2.2.2 Pause, Stop, Hide, owned by another pack.
...
- Severity follows the user's task. 2.3.1 is `critical` whenever a threshold is
  exceeded - it is a health risk, not an inconvenience. A colour-only error state
  on the field blocking submission is `critical`. An invisible input boundary on
  the primary form is `serious`. A colour-only status in a supplementary table is
  `moderate`. A pale footer icon is `minor`.
- 1.4.3 is never reported from this lane, at any severity, however obvious it
  looks.
- Evidence discipline: never invent a selector and never invent a ratio. Quote
  ...
  than no number, because a developer will act on it. Write findings like "the
  measured ratio between the `#e0e0e0` border of `#postcode` and the `#ffffff`
  field is 1.28:1, and that border is the only thing marking the field".
- Verdict: DECIDE when a measurement or the arithmetic settles it. FLAG when you
  are judging pixels with no measurement, or could not sample enough frames.
