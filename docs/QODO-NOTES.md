...
- **We changed:** Nothing in response to Qodo. The branch then carried nine further commits, found by running the product against a real target rather than by review:
  ...
  - **A patch in an unopened pull request scored as a passing criterion.** The score counted `fixed` and `verified` findings as closed, but both describe a *patch* — that FIX wrote one and it built and passed the target's tests — and neither is an observation about the page. A run that had recorded "2 failing, 8 findings" re-scored on resume as "0 failing, 8 findings": every finding still in the ledger, every criterion reported passing, nothing re-audited. Only a human's dismissal closes a finding now.
  - **An interrupted run could never be recovered.** `openDecideFindings` asks for `open` or `fixing`, so once VERIFY marked its findings `verified`, a run interrupted between verification and the pull request came back to an empty work list, ended at "No DECIDE findings to fix", and stranded the approval card with no conductor able to act on it. The patches decide now.
  - **A dropped connection decided a run.** The approval wait polls every two seconds and can run for minutes; a single `ECONNRESET` from the Supabase pooler threw straight out of the conductor and ended a run with the pull request one click away. It retries now and gives up only after five minutes of sustained outage. This one was observed live, not reasoned about.
  - **The final audit re-measured a site the fix had never reached.** With nothing serving the patched build, the only URL available is the deployed one, whose code is the unmerged base — so the pass spent minutes re-crawling to reach the number it started from. A zero delta there reads as evidence the fix did nothing. Skipped now, with the reason recorded.
  - **The pull-request body claimed a passing test suite over a red one.** `testsPassed` is baseline-aware — it means "no test this change touched regressed", not "every test is green" — and the body read it as the latter, writing "This repository's own test suite still passes" about a repository whose suite is red on `main`. A reviewer disproves that by running the tests once, in the one document whose purpose is to be believed. The body now carries the verifier's own sentence, which says the suite is red and that the red is not ours.
- **We dismissed:** Nothing. A re-review was requested with `/review` twice after the later commits landed and had not posted when this entry was written — Qodo's standing verdict covers the branch as it stood at the auth and landing-page changes, not the fourteen commits after it. That is recorded rather than presented as a fresh pass, and anything a later review raises will be appended here.
